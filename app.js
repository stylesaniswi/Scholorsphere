    ...
    try{
       ...
       const counselee = StudentRegister.findOne({registration_number:req.body.beneficiary});
        const remainder = new Allremainder({
            ...
            createdBy:counselloremail,
            beneficiaryemail:"anishshrestha607@gmail.com",
...
app.get("/chat",async(req,res)=>{
    try{
        ...
        const comments = forumcomment.comments;
        const commentos = await Comments.find({"createdAt":{$gte:new Date().toJSON().slice(0, 10)}});
        var score =0;
        commentos.forEach(function(s){
           score += sentiment.analyze(s.content).score;
        });
        ...
        sentimentscore: score/commentos.length
        });
        failure=false;
        msg="";
    }catch(error){
...
app.get("/usercounselling",async(req,res)=>{
    ...
    try{
        ...
        res.render("usercounselling",{failure:failure,msg:msg,registernumber:req.session.studentid,counsel:counsell,msgwithcounsel:messagewithcounsellor,appointment:appointment,user:user});
        failure=false;
        msg="";
    }catch(error){
        failure=true;
